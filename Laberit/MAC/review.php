...
function getOui($conn, $mac) // Verfifica si la MAC es Pequeña, Mediana o Grande.
{
...
function get_device($conn, $mac){
    ...
    $sql = "SELECT * FROM mac WHERE macPrefix=\"$ma_s\" UNION SELECT * FROM mac WHERE macPrefix=\"$ma_m\" UNION SELECT * FROM mac WHERE macPrefix=\"$ma_l\";"; // Reemplazo el Query SQL por un Storage Procedure.
    $stmt = $conn->prepare($sql); // Se prepara la Consulta.
    $stmt->execute(); // Se Ejecuta.
    if ($stmt->rowCount() > 0) // Si se Obtienen Resultados.
    {
        $row = $stmt->fetxh(PDO::FETCH_OBJ);
        $oui = $row->macPrefix;
        return $oui;
    }
    else
